(function(){
  ...
  // Fecha del evento: 18 de Octubre, 2026, 6:00 PM (hora local del ejemplo)
  const EVENT_DATE = new Date('2026-10-18T18:00:00-06:00').getTime();
  ...
  // Lee el link del invitado (ej. tusitio.com/?n=Fam.%20Martinez&p=2) y:
  // - Muestra "Pase reservado para: [nombre] ([p] personas)"
  // - Limita el <select> de personas para que no pueda pedir más pases de los asignados
  // - Precarga el nombre en el campo de texto
  // Si el link no trae esos parámetros, el formulario se queda genérico (como antes).
  function setupInvitadoDesdeLink(){
    const params = new URLSearchParams(window.location.search);
    const nombreInvitado = params.get('n');
    const pasesRaw = params.get('p');
    const pases = pasesRaw ? parseInt(pasesRaw, 10) : null;

    if (!nombreInvitado || !pases || pases < 1) {
      return; // Link genérico, sin personalizar
    }
